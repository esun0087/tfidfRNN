...
void CRnnLM::loadTFIDF(char *file)
{
    std::ifstream in(file);
    std::string line;
    printf("loading tfidf weights...\n");
    while (std::getline(in,line))
    {
        ...
        ss >> w >> weight;
        tf_idf.insert(std::make_pair(w,weight));
    }
}
...
void CRnnLM::computeNet(int last_word, int word)
{
    ...
    if (tfidf)
    {
    	...
    	if (word > 0)
    	{
    		...
    		w = vocab[word].word;
		    v =tf_idf[w];
		    for (a=0; a<layer1_size; a++) neu1[a].ac=v;
		}
		#endif
	    if (word>0)
	    {
	    	w = vocab[last_word].word;
	    	v =tf_idf[w];
	    	for (a=layer0_size-layer1_size;a<layer0_size;a++)neu0[a].ac*=v;
	    }  
	}
	#endif
    if (last_word!=-1) neu0[last_word].ac=1.0; 
    ...
    if (tfidf)
    {
        ...
        {
            a=last_word;
            std::string w = vocab[a].word;
            //if (a!=-1) neu1[b].ac += neu0[a].ac + (tf_idf[w] * syn0[a+b*layer0_size].weight); // insert tf idf weight here
...
void CRnnLM::learnNet(int last_word, int word)
{
    ...
    if (tfidf)
    {
    	if (word > 0)
    	{
    		std::string w = vocab[word].word;
		    float v = tf_idf[w];
		    for (a=layer0_size-layer0_size; a<layer0_size; a++) neu0[a].er=v;
		}
	}
	#endif
